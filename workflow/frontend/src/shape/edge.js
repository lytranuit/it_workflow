import editorStyle from "../utilities/defaultStyle";
  ...
  G6.registerEdge(
    "flow-polyline-round",
    {
      ...
      setState(name, value, item) {
        ...
        var cfg = item._cfg.model;
        path.attr("lineWidth", this.options.style.lineWidth);
        //console.log(name, value, cfg)
        if (name === "selected") {
          if (value) {
      ...
      drawLabel(cfg, group) {
        ...
        if (cfg.allowDelete) {
          const icon = group.addShape("image", {
            attrs: {
              img: require("../assets/icons/flow/cancel-icon.svg"),
              x: labelStyle.x + labelBBox.width / 2 + 2,
              y: labelStyle.y - 6,
              width: 10,
              height: 10,
              cursor: "pointer",
      ...
      getShapeStyle(cfg) {
        ...
        //console.log(cfg.revers)
        //console.log(style)
        if (cfg.active) {
          style = { ...style, stroke: "green" };
        } else if (cfg.reverse) style = { ...style, stroke: "red" };
