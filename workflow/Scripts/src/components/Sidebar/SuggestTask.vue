<template>
    <div class="container" id="suggest">
        <div class="h-100" v-if="url != null">
            <div class="text-center mb-2" v-if="!model.blocking && Object.keys(signatures).length > 0">
                <a href="#" class="btn-next" @click="save($event)">
                    Lưu lại
                </a>
            </div>
                ...
                <div class="card-body">
                    <div class="row g-0">
                        ...
                        <div class="col-3 order-first" style="border: 5px solid #d7d7d7; height: 640px; display: inline-block; padding: 10px">
                            ...
                            <div id="sign">
                                <div class="signature ui-draggable ui-draggable-handle" :data-id="signature.block_id" v-for="(signature,index) in signatures">
                                    <div class="d-inline-block">
                                        <img class="sign_image" :src="signature.image" style="width:90px;height:auto;" alt="...">
        ...
        computed: {
            ...
            signatures() {
                ...
                var blocks_approve = this.nodes.filter(function (item) {
                    return item.data_setting.blocks_esign_id == blocks_esign_id && item.clazz == 'approveTask';
                });
                var signatures = {};
                ...
                for (var block of blocks_approve) {
                    ...
                    if (activity.length > 0 && activity[0].executed)
                        continue;
                    var signnature = {
                        block_id: block.id,
                        name: "Block: " + block.label,
                        image: path + "/images/tick.png",
                    }
                ...
                loadingTask.promise.then(async function (pdf) {
                    ...
                    if (suggests) {
                        $("#sign .signature").each(function () {
                            ...
                            $("#pdf-viewer .signature[data-id='" + block_id + "']").css({
                            ...
                            $("#pdf-viewer .signature[data-id='" + block_id + "'] .sign_image").resizable();
        ...
        methods: {
            ...
            async save(e) {
                ...
                if ($("#suggest").length) {
                    ...
                    var resp = await $.ajax({ url: path + "/admin/api/updateactivity", data: model, type: "POST", dataType: "JSON" });
                    location.reload();
                }
                return;
            }
