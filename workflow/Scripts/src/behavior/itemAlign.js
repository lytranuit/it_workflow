        ...
        _addAlignLine(p) {
            ...
            if (p.horizontals) {
                each(p.horizontals, function (lineObj) {
                    ...
                    const shape = group.addShape('line', { attrs: $.extendext(true, 'replace', { x1, y1: line[1], x2, y2: line[1] }, lineStyle), capture: false });
                    lineArr.push(shape);
                })
            }
            if (p.verticals) {
                each(p.verticals, function (lineObj) {
                    ...
                    const shape = group.addShape('line', { attrs: $.extendext(true, 'replace', { x1: line[0], y1, x2: line[0], y2 }, lineStyle), capture: false });
                    lineArr.push(shape);
                })
            }
        },
